...
subplot(121)
%2d
    h_data = plot(Data_pose_vel(1,:),Data_pose_vel(2,:),'r.','markersize',10); hold on;
    h_att = [];
    att=[Data_pose_vel(1,end);Data_pose_vel(2,end)];
    h_att = scatter(att(1),att(2),150,[0 0 0],'d','Linewidth',2); hold on;
    
    % Plot Velocities of Reference Trajectories
    vel_points = Data_pose_vel([1 2 4 5],1:vel_samples:end);
