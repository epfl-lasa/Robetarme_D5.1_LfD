...
for k=1:K
    ...
    pos_data = Data_k{k}(1:N,:);
    vel_data = Data_k{k}(N+1:2*N,:);
    dist_global_att = find(radius_fun(Data_k{k}(1:N,:))' > 0.5);%%---- wr: this can influence the dir of local attractor
    ...
    %%%---- wr: for circle DS, it can't get right direction , so:
    fprintf('%d-th Gaussian ends at the loacl attractor\n', k);
    local_dir = -(pos_data(:,1)-Mu(:,k))/norm(pos_data(:,1)-Mu(:,k));
    local_dirs(:,k)  = local_dir;
    att_l(:,k)   = pos_data(:,1);   
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
