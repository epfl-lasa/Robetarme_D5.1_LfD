...
pkg_dir = '/home/rui/MATLAB_ON_LINUX/8_Learn_DS_cut_task/lagsDS-opt/';
%%%%%%%%%%%%%%%%%%% Choose a Dataset %%%%%%%%%%%%%%%%%%%%%                     
    ...
    %%%%%%%%  LPV-DS f_g(xi) = sum_{k=1}^{K}\gamma_k(xi)(A_kxi + b_k) %%%%%%%%                
        switch lyapType
            ...
            case 1
                ...
                enforce_g = 0;  % ==> "Enforce A+A' If local constraints not met
...
switch act_type
    ...
    case 1 % With GPR      
        ...
        % Parametrize GPR
        epsilon = 0.1; % assuming output variance = 1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
kappas = [1 1 1 15 2 1 1 15 1]; % Relative ratio between eigenvalues
% kappas = [1 10 10 1 10 1 1]; % Relative ratio between eigenvalues
...
for k=1:K    
    if any(find(choosen_active==k))
        ...
        [A_l, ~, ~, ~] = estimate_localDS_known_gamma(Data_k, A_g(:,:,k),  att_g, att_l(:,k), 1, kappas(k) ,w(:,k), P_g, P_l(:,:,k), local_basis(:,:,k));
        b_l = -A_l*att_l(:,k);
        ...
        % Constraint Checks
        Q_l  = A_l'*P_l(:,:,k);
        lambda_Ql  = eig(Q_l+Q_l')
        Q_lg = 2*A_l'*P_g;
        lambda_Qlg = eig(Q_lg+Q_lg')        
