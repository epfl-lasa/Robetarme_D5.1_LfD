...
%     cd /home/rui/matlab
    ...
    rmpath(genpath('8_Learn_DS_cut_task'));
    addpath(genpath('8_Learn_DS_cut_task/e-lagsDS-run-only-active-inside-this-file'))
    cd 8_Learn_DS_cut_task/e-lagsDS-run-only-active-inside-this-file/
else
...
%%%%%%%%%%%%%%%%%%%%%%%%% Select a Dataset %%%%%%%%%%%%%%%%%%%%%%%%%%
% 1:  LASA_Cut_dataset_2D_1127_hard   (2D) *
...
% 12: 3D soft cut whole process
% 13: 3Dhard cut (without leave process)
% 14: 2D cut hard (without leave process)
% 15 16: 3D and 2D cut with out smooth the vel
% 17 18�?D and 2D cut with orign data without interp
% 19 the 2D cut with filter of position and vel
% 23 linear with curve
% 24 LASA_3D_curve_cut_for_lagsDS_avocado_good
% 25 LASA_3D_curve_cut_for_lagsDS_orange_good
% 26 LASA_3D_curve_cut_for_lagsDS_plastkc_good
% 27 LASA_3D_curve_cut_for_lagsDS_tissue10_good
% 28 use skull polish data with phase1 and phase2 (3D)
% 29 use skull polish data only phase2 (3D)
% 30 use egg_cut_phase2 only phase2 (3D)
% 31 use skull polish for force-vel-DS phase12 (3D)
% 32 use egg_cut old demo for force-vel-DS phase12 (3D)
% 33 use egg_cut new demo for force-vel-DS phase12 (3D)
% 34 use skull polish new tool phase12 (3D)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pkg_dir_load     = './ds-opt';
pkg_dir_save     = './lagsDS-opt-rui';

% DS_name         ='orange3D';
% DS_name         ='egg_cut_phase2';
% DS_name         ='skull_polish_phase12';
% DS_name         ='egg_cut_old_phase12';
% DS_name         ='egg_cut_new_phase12';
DS_name         ='skull_new_tool_phase12';
chosen_dataset  = 34; 
sub_sample      = 5; % '>2' for real 3D Datasets, '1' for 2D toy datasets
nb_trajectories = 7; % For real 3D data only
...
est_options.sub_sample       = 15;   % Size of sub-sampling of trajectories
                                    % 1/2 for 2D datasets, >2/3 for real    
% Metric Hyper-parameters
est_options.estimate_l       = 1;   % '0/1' Estimate the lengthscale, if set to 1
est_options.l_sensitivity    = 16;   % lengthscale sensitivity [1-10->>100]----wr: this can chahge gaussion num
...
hold on;
scatter(Data_mean(1,:),Data_mean(2,:),Data_mean(3,:), 'k*')
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% DS_name = 'wr-try-lags';
save_lpvDS_to_Mat(DS_name, pkg_dir_load, ds_gmm, Data, Data_sh, data, A_g, b_g, att_g, x0_all, dt, P_g, 2, est_options)

%% Save LPV-DS parameters to text files
% DS_name = 'wr-try-lags';
save_lpvDS_to_txt(DS_name, pkg_dir_load,  ds_gmm, A_g, att_g)
...
save_lpvDS_to_Yaml(DS_name, pkg_dir_load,  ds_gmm, A_g, att_g, x0_all, dt)
...
%     tracking_factor = 30*ones(1,K); % Relative ratio between eigenvalues    
    tracking_factor = scale*ones(1,K)*15;
    tracking_factor = scale*ones(1,K)*15;
else
...
tracking_factor(choosen_active) = 10000
% tracking_factor(3) = 2000
% tracking_factor(4) = 50
tracking_factor=[100   1000   100   100   1000   1000   100   100   100 100];
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%% option add by wr:Visualize DS Streamline in 2D axis %-------------------------------
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% [hd, hs, hr, x_sim] = visualizeEstimatedDS(Xi_ref, lags_ds, ds_plot_options);
...
%%% Plot ZX coordinates of DS
xz = [1 3];
ds_plot_options = [];
ds_plot_options.sim_traj   = 1;      % To simulate trajectories from x0_all
ds_plot_options.x0_all     = x0_all; % Initial Points for reproductions
ds_plot_options.dimensions = xz;
ds_plot_options.attractor  = att + [0 0 0.005]';
[hd, hs, hr, x_sim] = visualizeEstimatedDS(Xi_ref, lags_ds, ds_plot_options);
title('XZ-slice of learned DS', 'Interpreter','LaTex','FontSize',20)
%% %% plot 3D tube
fig_lagds_tube = figure('Color',[1 1 1]);
[hds] = plot_ds_model_and_tube_3D(fig_lagds_tube, lags_ds, att_g, limits,x0_all','low');

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%      [OPTIONAL]: WRITE LAGS PARAMETERS TO mat FORMAT FOR simulation   %%      
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% save workspace incase lost data
save([pkg_dir_save '/models/' DS_name],'limits','Xi_ref','x0_all','att','alpha_fun',...
    'ds_gmm', 'A_g', 'b_g', 'A_l_k', 'b_l_k', 'A_d_k', 'b_d_k', 'h_functor',...
    'lambda_functor', 'grad_h_functor', 'att_g', 'att_l', 'modulation', 'scale');
% load('lagsDS-opt-rui/good_case_using_curve_data_train_lagds');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%    orign option:  Create Function for DS and PLot                     %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
modulation = 0; do_scale = 1;
% lags_ds = @(x) lags_ds_nonlinear(x, alpha_fun, d_gmm, 1/scale*A_g, 1/scale*b_g, A_l_k, b_l_k, A_d_k, b_d_k, h_functor, lambda_functor, grad_h_functor, att_g, att_l, modulation, scale, do_scale);
lags_ds = @(x) lags_ds_nonlinear(x, alpha_fun, ds_gmm, 1/scale*A_g, 1/scale*b_g, A_l_k, b_l_k, A_d_k, b_d_k, h_functor, lambda_functor, grad_h_functor, att_g, att_l, modulation, scale);
...
save_lagsDS_to_txt(DS_name, pkg_dir_save,  ds_gmm, (1/scale)*A_g, att_g, A_l_k, A_d_k, att_l, w, breadth_mod*ones(1,K), scale, B_r)
...
model_dir = strcat(pkg_dir_save,'/models/',DS_name, '/');
...
% To use the rest of the code you need a matlab yaml convertor
% you can get it from here: http://vision.is.tohoku.ac.jp/~kyamagu/software/yaml/
save_lagsDS_to_Yaml(DS_name, pkg_dir_save,  ds_gmm, (1/scale)*A_g, att_g, x0_all, dt, A_l_k, A_d_k, att_l, w, breadth_mod*ones(1,K), scale, B_r, gpr_filename)
...
Xrange_values = [4;4];
Data_grid = Xmin_values(:,ones(1,N_samples)) + rand(N_samples,M)'.*(Xrange_values(:, ones(1,N_samples)));
Data_grid(M+1:M+2,:) = zeros(M, length(Data_grid));

% Simulate velocities from same reference trajectory
Data_test = [Data Data_grid];
% Data_test = [Data ];
xd_dot_global = global_ds(Data_test(1:M,:));    
xd_dot_local  = lags_ds_local(Data_test(1:M,:));
xd_alpha      = alpha_fun(Data_test(1:M,:));
xd_dot_lags   = lags_ds(Data_test(1:M,:));
