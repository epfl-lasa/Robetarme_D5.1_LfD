function plotGMM_3D(Mu, Sigma, color, display_mode);
%
...
if display_mode==1
  ...
  for j=1:nbData
    stdev = sqrtm(3.0.*Sigma(:,:,j));%mean square root
    X = [cos(t) sin(t)] * real(stdev) + repmat(Mu(:,j)',nbDrawingSeg,1);%real:qu shi bu.
...
%   end
    ...
    hold on; 
    plot3(Mu(1,1), Mu(2,1), Mu(3,1),'g.','markersize',20);hold on;
    plot3(Mu(1,end), Mu(2,end), Mu(3,end),'r*','markersize',20);
    view(-85,53); set(gca,'proj','perspective'); grid on;
  grid on;  axis tight;
  
%this part used to get the trajectory 
  Data_trj=zeros(3,length(Mu));
for i=1:length(Mu)  
    Data_trj(1,i)=Mu(1,i);
    Data_trj(2,i)=Mu(2,i);  
    Data_trj(3,i)=Mu(3,i);  
end  
save GMR_trj.mat Data_trj

  
  elseif display_mode==5%how to plot a ellipsoid : https://blog.csdn.net/liuxqsmile/article/details/1893411
  for j=1:nbData
      h = plot_gaussian_ellipsoid( Mu(:,j)', Sigma(:,:,j)',2.5);
  end
  view(-85,53); set(gca,'proj','perspective'); grid on;
