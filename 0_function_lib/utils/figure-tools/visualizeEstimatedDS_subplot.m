function [fig1,hd, hs, hr, x_sim] = visualizeEstimatedDS_subplot(Xi_ref, ds_fun, ds_plot_options)
...
if isfield(ds_plot_options,'plot_demo_data')
    plot_demo_data = ds_plot_options.plot_demo_data;
else
    plot_demo_data = 1;
end
...
elseif M == 3
    ...
    if plot_2D_only
        ...
        [hd] = scatter(Xi_ref_2d(1,:),Xi_ref_2d(2,:),10,color_r,'filled'); hold on
        ...
        [hs] = plot_ds_model_3D_2Dslice(fig1, ds_fun, attractor, limits_, dimensions, 'high'); hold on;       
        axis(limits_)                
        box on
        grid on
        ...
        % Plot Demonstrations in red
        [hd] = plot3(Xi_ref(1,:),Xi_ref(2,:),Xi_ref(3,:),'Color',color_r,'markersize',10); hold on;
