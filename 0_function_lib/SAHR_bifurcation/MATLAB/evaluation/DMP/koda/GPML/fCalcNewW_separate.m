function nwW=fCalcNewW_separate(dmW,dmQ,Hyp,nwQ,weight_separate)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%% Hyp - GPR Hyper Parameters
%%% nwQ - new querry point
%%%%%
...
GPsett=Hyp{1}{weight_separate}.GPsett; %%% GPR setting used for Hyp Params Calc (same over all Dof and W);

%     covfunc = {'covSum',{cn,cga}}; initHyp.cov = [hypn; hypga];
%     meanfunc = mc; initHyp.mean = hypc;
%     %meanfunc = mp; initHyp.mean = hypp;
%     likfunc = lr0; initHyp.lik = hypr0;
    

disp('>>> CALCULATING NEW WEIGHTS >>>')
for cDof=1:nDof;
    for cW=weight_separate;
        
        nwW(cDof,cW)=gp(Hyp{cDof}{cW}, @infExact,  GPsett.MF, GPsett.CF, GPsett.LF, dmQ', squeeze(dmW(cDof,cW,:)), nwQ);
        %nwW(cDof,cW)=gp(Hyp{cDof,cW}, @infExact,  meanfunc, covfunc, likfunc, dmQ', squeeze(dmW(cDof,cW,:))', nwQ);
